import React, { useState } from "react";
import {
  Card,
  CardBody,
  CardFooter,
  Typography,
  Button,
  Dialog,
  DialogHeader,
  DialogBody,
  DialogFooter,
} from "@material-tailwind/react";
import {
  FaGlobe,
  FaLaptopCode,
  FaPaintBrush,
  FaDatabase,
} from "react-icons/fa";

const CardProject = () => {
  const cardContent = [
    {
      title: "Project Website Fullstack",
      description:
        "Sistem pemesanan tour dengan fitur booking online, autentikasi pengguna, dan integrasi Midtrans.",
      icon: <FaGlobe className="text-customBlue w-12 h-12 mb-4" />,
      link: "https://github.com/your-username/fullstack-project",
      dialogContent: (
        <div>
          <p className="mb-2 text-gray-600 font-poppins">
            Dibangun dengan React, Node.js, Express, dan MySQL. Sistem ini
            memungkinkan pengguna untuk:
          </p>
          <ul className="list-disc list-inside text-sm text-gray-600 font-poppins">
            <li>Melakukan registrasi dan login</li>
            <li>Melakukan booking paket tour</li>
            <li>Melakukan pembayaran melalui Midtrans</li>
            <li>Mendapatkan invoice secara otomatis</li>
          </ul>
        </div>
      ),
    },
    {
      title: "Project Website Frontend",
      description:
        "Pengembangan antarmuka web interaktif menggunakan React.js dan Tailwind CSS.",
      icon: <FaLaptopCode className="text-customBlue w-12 h-12 mb-4" />,
      link: "https://github.com/your-username/frontend-project",
      dialogContent: (
        <div>
          <p className="mb-2 text-customBlue">
            Fokus pada pembuatan antarmuka responsif dan user-friendly dengan:
          </p>
          <ul className="list-disc list-inside text-sm text-customBlue">
            <li>React functional components</li>
            <li>Tailwind CSS utility-first styling</li>
            <li>Routing menggunakan React Router</li>
            <li>State management dengan useState dan Context</li>
          </ul>
        </div>
      ),
    },
    {
      title: "Project UI Bali Pure Tour",
      description:
        "Rancangan tampilan user interface (UI) untuk sistem tour Bali Pure Tour menggunakan Figma.",
      icon: <FaPaintBrush className="text-customBlue w-12 h-12 mb-4" />,
      link: "https://www.figma.com/file/your-file-id/Bali-Pure-Tour-UI",
      dialogContent: (
        <div>
          <p className="mb-2 text-customBlue">
            UI dirancang agar mudah digunakan oleh pengguna awam:
          </p>
          <img
            src="https://via.placeholder.com/400x200"
            alt="UI Bali Pure Tour"
            className="rounded-lg shadow-md"
          />
        </div>
      ),
    },
    {
      title: "Project UI Traveling Kuy",
      description:
        "Rancangan tampilan user interface (UI) untuk sistem tour Traveling Kuy menggunakan Figma.",
      icon: <FaPaintBrush className="text-customBlue w-12 h-12 mb-4" />,
      link: "https://www.figma.com/file/your-file-id/Traveling-Kuy-UI",
      dialogContent: (
        <div>
          <p className="mb-2 text-customBlue">
            Desain ini berfokus pada UX mobile-first:
          </p>
          <ul className="list-disc list-inside text-sm text-customBlue">
            <li>Warna cerah dan bersih</li>
            <li>Fitur booking cepat</li>
            <li>Live preview harga & tanggal</li>
          </ul>
        </div>
      ),
    },
    {
      title: "Project Backend API",
      description:
        "Pembuatan REST API untuk sistem booking tour menggunakan Node.js, Express, MySQL, dan MongoDB.",
      icon: <FaDatabase className="text-customBlue w-12 h-12 mb-4" />,
      link: "https://github.com/your-username/backend-api",
      dialogContent: (
        <div>
          <p className="mb-2 text-customBlue">
            Backend ini menyediakan endpoint untuk:
          </p>
          <ul className="list-disc list-inside text-sm text-customBlue">
            <li>Autentikasi pengguna (JWT)</li>
            <li>CRUD paket tour & booking</li>
            <li>Integrasi Midtrans & notifikasi</li>
            <li>Data analitik dan laporan</li>
          </ul>
        </div>
      ),
    },
  ];

  const [openDialogIndex, setOpenDialogIndex] = useState(null);

  const handleOpen = (index) => {
    setOpenDialogIndex(index);
  };

  const handleClose = () => {
    setOpenDialogIndex(null);
  };

  return (
    <div className="py-12 px-4 lg:px-20">
      <div className="mb-10">
        <Typography
          color="blue-gray"
          variant="lead"
          className="font-semibold text-base font-poppins sm:text-lg lg:text-2xl"
        >
          My Projects
        </Typography>
        <p className="mt-2 text-lg text-gray-600 leading-snug font-poppins">
          Completed real-world web development projects
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {cardContent.map((card, index) => (
          <Card
            key={index}
            className="shadow-lg hover:shadow-xl transition font-poppins"
          >
            <CardBody className="font-poppins">
              {card.icon}
              <Typography
                variant="h5"
                color="blue-gray"
                className="mb-2 font-poppins"
              >
                {card.title}
              </Typography>
              <Typography className="text-[16px] font-poppins text-gray-600">
                {card.description}
              </Typography>
            </CardBody>
            <CardFooter className="pt-0 font-poppins">
              <Button
                size="sm"
                variant="text"
                onClick={() => handleOpen(index)}
                className="flex items-center gap-2 font-poppins text-[16px]" // Hapus mx-auto
              >
                Learn More
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={2}
                  stroke="currentColor"
                  className="h-4 w-4"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M17.25 8.25L21 12m0 0l-3.75 3.75M21 12H3"
                  />
                </svg>
              </Button>
            </CardFooter>

          </Card>
        ))}
      </div>

      {/* Dialog Section */}
      {openDialogIndex !== null && (
        <Dialog
          open={true}
          handler={handleClose}
          animate={{
            mount: { scale: 1, y: 0 },
            unmount: { scale: 0.9, y: -100 },
          }}
        >
          {/* <DialogHeader>{cardContent[openDialogIndex].title}</DialogHeader>
          <DialogBody>{cardContent[openDialogIndex].dialogContent}</DialogBody> */}
          <DialogHeader className="font-poppins -mb-5">
            {cardContent[openDialogIndex].title}
          </DialogHeader>
          <DialogBody className="font-poppins">
            {cardContent[openDialogIndex].dialogContent}
          </DialogBody>
          <DialogFooter>
            <Button
              variant="text"
              color="red"
              onClick={handleClose}
              className="mr-2 font-poppins"
            >
              <span>Close</span>
            </Button>
            <a
              href={cardContent[openDialogIndex].link}
              target="_blank"
              rel="noopener noreferrer"
            >
              <Button variant="gradient" color="green">
                <span>Open</span>
              </Button>
            </a>
          </DialogFooter>
        </Dialog>
      )}
    </div>
  );
};

export default CardProject;
